...
import { DoorOpen, UserPlus, ArrowLeft, ArrowRight } from 'lucide-react';
...
export const RoomGrid: React.FC<RoomGridProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Rooms Grid: 3-column layout matching the drawing */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-3.5 sm:gap-4">
        {roomCards.map((room) => (
          <div
            ...
            className="group relative bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200/90 dark:border-slate-700/90 shadow-sm hover:shadow-lg hover:border-blue-500/50 dark:hover:border-blue-500/50 transition-all duration-200 flex flex-col justify-between"
          >
            <div>
              {/* Room Card Header */}
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-700/60">
                ...
                <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${room.badgeClass}`}>
                  {room.statusText}
                </span>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between gap-2">
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 px-3 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/50 rounded-lg transition-colors"
              ...
              {canEdit && room.count < room.max && onAddWorkerToRoom && (
                <button
                  ...
                  className="p-2 text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 rounded-lg transition-colors"
